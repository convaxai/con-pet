...
Con Pet 是一个 Windows / macOS 桌面常驻应用：当用户在任意应用中输入配置的键盘指令时，在屏幕上播放一段透明、鼠标穿透的宠物动画。

## 当前能力

- 支持录入由组合键和连续按键构成的结构化指令。默认指令为 `M → J`；例如 macOS 的 `⌘ + C`、Windows 的 `Ctrl + C`，以及两个平台都支持的 `Space → Space`。
- 保存稳定的物理按键标识，不读取上屏文字。长按不会被算作多次按键，修饰键必须精确匹配，未完成的序列会在配置的间隔后失效。
- 使用跨平台“主修饰键”：macOS 对应 Command，Windows 对应 Control；平台特有的 Control / Windows 键组合仍会区分。
- 全局监听不会注册、替换或吞掉已有快捷键。复制等原操作仍会执行，Con Pet 只会同时响应；部分系统保留指令可能不会被系统交给应用观察。
- 随包内置 100 个热门 Codex Pets，离线即可使用；也会自动发现 `~/.codex/pets/*/pet.json`。
...
标准图集为 `1536×1872`，共 8 列 × 9 行，每格 `192×208`。动作行依次为：待机、向右跑、向左跑、挥手、跳跃、失败、等待、小跑、检查。也兼容 Codex Pets 当前使用的 8×10 / 8×11 扩展图集，不会纵向压缩。内置蜘蛛侠动态彩蛋使用 8×11 图集连续存放 83 个源帧。
